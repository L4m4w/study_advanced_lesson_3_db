...
class TestRegressApi:
    ...
    @pytest.mark.regresin
    def test_get_users_positive_response(self):
        response = requests.get(f"{self.BASE_URL}/users", headers=self.headers)
        assert response.status_code == 200

    @pytest.mark.regresin
    @pytest.mark.xfail(reason='Flaky http method, without authorization it could return 200')
    def test_get_users_unauthorized_response(self):
        response = requests.get(f"{self.BASE_URL}/users?page=2&per_page=3321")
